...
for (k in 1:pvar) {
  for (i in 1:N) {
    for (j in 1:N) {
      Q_all[i,j, k] = kernalGaussian(Xfun[i, which(label_left[,2] == c_reg[k])], 
                                     Xfun[j, which(label_left[,2] == c_reg[k])], para = h)
    }
  }
}
...
for (k in 1:pvar) {
  for (i in 1:N) {
    for (j in 1:N) {
      Q_all[i,j, k] = kernalLap(Xfun[i, which(label_left[,2] == c_reg[k])], 
                                Xfun[j, which(label_left[,2] == c_reg[k])], para = h)
    }
  }
}
