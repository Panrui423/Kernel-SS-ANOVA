...
// [[Rcpp::export]]
extern "C" SEXP MetricKest_general(const MatrixXd& dxtest, int train_num, int test_num) {
  MatrixXd out = MatrixXd::Zero(test_num, test_num);
  ...
  for(int i = 0; i < test_num; i++) {
    for(int j = 0; j < test_num; j++) {
      ...
      for(int k = 0; k < train_num; k++){
        VectorXd dxtest_row = dxtest.row(k);
        VectorXd vec1 = Map_Expball( dxtest(i + train_num, k), dxtest_row);
        VectorXd vec2 = Map_Expball( dxtest(j + train_num, k), dxtest_row);
        element += (vec1.transpose() * vec2);
      } 
