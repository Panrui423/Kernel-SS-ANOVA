...
simulation_K <- function(n.fit, n.test, h, scale, sigma){
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      Q[i,j,1] = kernalGaussian(X[i,1:100], X[j,1:100], para = 0.1) 
    }
  }
  
  for (i in 1:n) {
    for (j in 1:n) {
      Q[i,j,2] = kernalGaussian(X[i,101:250], X[j,101:250], para = 0.1) 
    }
  }
  
  for (i in 1:n) {
    for (j in 1:n) {
      Q[i,j,3] = kernalGaussian(X[i,251:450], X[j,251:450], para = 0.1) 
    }
  }
