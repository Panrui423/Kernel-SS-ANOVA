...
count_m <- function(){
  ...
  X_il_w <- c()
  for (i in 1:3) {
      v <- vec(R(e_3,u_i1[i]) * R(e_2,u_i2[i]) * R(e_3,u_i3[i]))
...
simulation_M <- function(n.fit, n.test, h, scale, sigma){
  ...
  for (l in 1:length(scale)) {
    ...
    time1 <- Sys.time()
    runningtime1[l] <- (time1-timestart_2) + (timeend_4-timestart_1)
    ...
    fit_ms_m <- tryCatch(gss:::mspreg1(s = s,r=Kermat_train[,id.basis,],id.basis=id.basis,method = 'm',
    ...
    { 
      ...
      for (i in 1:6) {
        M <- 10^theta_m[i] * Kermat_train[,,i] + M
      }
      
      hat_m <- c(M %*% c_m) + scalar[1:n.fit] * d_m
      fit_error_m1[l] <- mean((hat_m-y_M[1:n.fit])^2)
      ...
      predict_error_m1[l] <- mean((p_y_m-y_M.test)^2)
    }
  ...
  s = matrix(scalar[1:n.fit], n.fit, 1)
  fit_ms_m <- tryCatch(gss:::mspreg1(s = s, r=Q_M[,1:n.fit,],id.basis=1:n.fit,method = 'm',
                            y = y_M[1:n.fit],wt=1,alpha = 1.4,random = NULL,varht=1,skip.iter=FALSE),
                            error=function(x) NA)
  if((any(is.na(fit_ms_m))))
  { 
  ...
  fit_error_m1_no <- mean((hat_m-y_M[1:n.fit])^2)
  ...
  predict_error_m1_no <- mean((p_y_m-y_M.test)^2)
  }
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      Q[i,j,1] = kernalGaussian(X[i,1:100], X[j,1:100], para = 1)
    }
  }
  
  for (i in 1:n) {
    for (j in 1:n) {
      Q[i,j,2] = kernalGaussian(X[i,101:250], X[j,101:250], para = 1)
    }
  }
  
  for (i in 1:n) {
    for (j in 1:n) {
      Q[i,j,3] = kernalGaussian(X[i,251:450], X[j,251:450], para = 1)
    }
  }
  ...
  mat <- matrix(c(runningtime1/60, runningtime2/60,
                  ...
                  fit_error_m4, predict_error_m4), (10 + length(scale)*6), 1) # In mins
